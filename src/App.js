      ...
      <div id="card" ref={cardRef} style={{ maxWidth: 1040, margin: "0 auto", boxShadow: "0 16px 64px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: `3px solid ${GOLD}`, columnGap: 4, background: NAVY }}>
          <div style={{ background: TAN, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            <img src={data.left.photo} alt={data.left.name} style={{ width: 60, height: 72, objectFit: "cover", objectPosition: "top center", flexShrink: 0 }} />
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontFamily: serif, fontSize: "clamp(18px, 3vw, 28px)", fontWeight: 700, color: "#2A2520", lineHeight: 1.1, wordBreak: "break-word" }}>{data.left.name}</div>
              <div style={{ fontFamily: body, fontStyle: "italic", fontSize: "clamp(10px, 1.5vw, 12px)", color: MED_BROWN, marginTop: 3 }}>{data.left.role}</div>
            </div>
          </div>
          <div style={{ background: DARK_NAVY, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            <img src={data.right.photo} alt={data.right.name} style={{ width: 60, height: 72, objectFit: "cover", objectPosition: "top center", flexShrink: 0 }} />
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontFamily: serif, fontSize: "clamp(18px, 3vw, 28px)", fontWeight: 700, color: WHITE, lineHeight: 1.1, wordBreak: "break-word" }}>{data.right.name}</div>
              <div style={{ fontFamily: body, fontStyle: "italic", fontSize: "clamp(10px, 1.5vw, 12px)", color: GOLD, marginTop: 3 }}>{data.right.role}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: `2px solid ${GOLD}`, columnGap: 4, background: NAVY }}>
          <div style={{ background: LIGHT_TAN, padding: "6px 12px" }}>
            <div style={{ fontFamily: body, fontSize: "clamp(8px, 1.2vw, 11px)", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: GOLD }}>{data.left.colHeader}</div>
          </div>
          <div style={{ background: NAVY, padding: "6px 12px" }}>
            <div style={{ fontFamily: body, fontSize: "clamp(8px, 1.2vw, 11px)", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#8A3A2A" }}>{data.right.colHeader}</div>
          </div>
        </div>
